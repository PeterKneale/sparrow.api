...
type (
	// ReadAccountCommand is the command line data structure for the read action of Account
	ReadAccountCommand struct {
		PrettyPrint bool
	}
...
// Run makes the HTTP request corresponding to the ReadAccountCommand command.
func (cmd *ReadAccountCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ReadAccount(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
func (cmd *ReadAccountCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
}
